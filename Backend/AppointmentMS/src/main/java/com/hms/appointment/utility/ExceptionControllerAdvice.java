...
public class ExceptionControllerAdvice {
    ...
    public ResponseEntity<ErrorInfo> hmsExceptionHandler(HmsException e) {
        ErrorInfo error = new ErrorInfo(environment.getProperty(e.getMessage()), (long) HttpStatus.INTERNAL_SERVER_ERROR.value(), LocalDateTime.now());
        return new ResponseEntity<>(error, HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    public ResponseEntity<ErrorInfo> handleValidationExceptions(Exception e) {
        ...
        ErrorInfo error = new ErrorInfo(errorMsg, (long) HttpStatus.BAD_REQUEST.value(), LocalDateTime.now());
